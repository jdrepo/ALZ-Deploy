metadata name = 'ALZ Bicep - Ubuntu HA Module'
metadata description = 'ALZ Bicep Module used to set up Ubuntu in HA mode'
...
param parVirtualMachineSize string = 'Standard_B1sm'

@sys.description('Ubuntu Primary NVA Machine Name')
param parPrimaryVirtualMachineName string

@sys.description('Ubuntu Secondary NVA Machine Name')
...
param parUntrustedSubnetName string = 'NVA-Untrusted'

@sys.description('Trusted-Subnet Name.')
param parTrustedSubnetName string = 'NVA-Trusted'
...
param parShellScriptName string = 'configure-ubuntu-nva-ha.sh'

@sys.description('Install NVA Primary with CustomScript extension')
param parConfigurePrimaryNva string = 'yes'

@sys.description('Install NVA Secondary with CustomScript extension')
param parConfigureSecondaryNva string = 'yes'
...
var varElbPublicIPAddressName = 'pip-${parLocationCode}-elb-ubuntu-${parCompanyPrefix}-${varEnvironment}'
...
module modNsgNvaTrustedSubnet 'br/public:avm/res/network/network-security-group:0.5.1' =  {
  name: '${_dep}-nsg-ubuntu-trusted-subnet'
  params: {
...
module modNsgNvaUntrustedSubnet 'br/public:avm/res/network/network-security-group:0.5.1' = {
  name: '${_dep}-nsg-ubuntu-untrusted-subnet'
  params: {
...
module modElb 'br/public:avm/res/network/load-balancer:0.4.2' = {
  name: '${_dep}-elb-ubuntu-nva'
  params: {
    name: 'elb-${parLocationCode}-ubuntu-nva'
    ...
    backendAddressPools: [
      {
        ...
        loadBalancerBackendAddresses: [
          {
            name: 'ubuntu-nva-primary-untrusted'
            properties: {
          ...
          {
            name: 'ubuntu-nva-secondary-untrusted'
            properties: {
    ...
    inboundNatRules: [
       {
        backendAddressPoolName: 'ubuntu-nva'
        backendPort: 22
        frontendIPConfigurationName: 'FW'
        frontendPortRangeEnd: 51122
        frontendPortRangeStart: 51022
...
module modIlb 'br/public:avm/res/network/load-balancer:0.4.2' = {
  name: '${_dep}-ilb-ubuntu-nva'
  params: {
    name: 'ilb-${parLocationCode}-ubuntu-nva'
    frontendIPConfigurations: [
      {
        name: 'FW'
    ...
    backendAddressPools: [
      {
        ...
        loadBalancerBackendAddresses: [
          {
            name: 'ubuntu-nva-primary-trusted'
            properties: {
          ...
          {
            name: 'ubuntu-nva-secondary-trusted'
            properties: {
...
// module ubuntu-nva-primary 

module modUbuntuNvaPrimary 'br/public:avm/res/compute/virtual-machine:0.15.0' = {
  name: '${_dep}-ubuntu-nva-primary'
  dependsOn: [
    modKv
    modPrimaryKvPassword
  ]
  params: {
    ...
    extensionCustomScriptConfig: (parConfigurePrimaryNva == 'yes') ? {
      enabled: true
      fileData: [
        {
    ...
    extensionCustomScriptProtectedSetting: (parConfigurePrimaryNva == 'yes') ? {
        commandToExecute: 'sh ${parShellScriptName} ${parNvaScriptURI}'
    } : {}
  }
}

// module ubuntu-nva-secondary

module modUbuntuNvaSecondary 'br/public:avm/res/compute/virtual-machine:0.15.0' = {
  name: '${_dep}-ubuntu-nva-secondary'
  dependsOn: [
    modKv
    modSecondaryKvPassword
  ]
  params: {
    ...
    bootDiagnostics: true
    bootDiagnosticStorageAccountName: modSaBootDiag.outputs.name
    extensionCustomScriptConfig: (parConfigureSecondaryNva == 'yes') ? {
      enabled: true
      fileData: [
        {
          uri: '${parNvaScriptURI}${parShellScriptName}'
    ...
    extensionCustomScriptProtectedSetting: (parConfigureSecondaryNva == 'yes') ? {
        commandToExecute: 'sh ${parShellScriptName} ${parNvaScriptURI}'
    } : {}
  }
}
