...
$cert = Get-ChildItem -path Cert:\* -Recurse | Where-Object {$_.Subject -eq "CN=$certSubject"}
...
@(
 ...
 '-----END CERTIFICATE-----'
) | Out-File -FilePath w:/nginx/ssl/nginx-backend-tls.crt -Encoding ascii
...
@(
 ...
 '-----END RSA PRIVATE KEY-----'
) | Out-File -FilePath w:/nginx/ssl/nginx-backend-tls.key -Encoding ascii

# Create home page.
Invoke-WebRequest 'https://raw.githubusercontent.com/jdrepo/ALZ-Deploy/main/workloads/workload-online1/index.html' -OutFile 'w:/nginx/html/index.html'
